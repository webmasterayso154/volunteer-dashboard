...
## 2. Managing Caps & Season Rules (`Admin_Config`)

All point thresholds and category caps are managed dynamically from the **`Admin_Config`** tab in the main Google Spreadsheet. Any changes made here automatically propagate across both the Board Portal and the public coach view without editing code.
...
> **Administrative Note:** Never delete column headers in the `Admin_Config` tab. Modify only the numeric values in the adjacent setting value cells.
...
* **`SLOT_COLLISION` (Conflicting Center Referee):**
...
* **Adding Late Coaches:** Open the **`Admin_Config`** tab and navigate to Columns G through I (**Late-Added Coaches**). Enter the Division, Team Code, and Head Coach Name. The Apps Script engine merges these entries with `MASTER_TEAMS` dynamically.
* **Team Directory Sync:** Once a new team is saved in `Admin_Config`, coaches and team managers will immediately appear in the dropdown menu on both the public dashboard and internal portal.
